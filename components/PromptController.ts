...
export class PromptController extends LitElement {
  static override styles = css`
    ...
    .prompt.midi-active {
      box-shadow: 0 0 1.5vmin 0.5vmin #ffffffaa;
    }
    weight-knob {
      width: 70%;
      flex-shrink: 0;
  ...
  private midiActiveTimeout?: number;
  ...
  override disconnectedCallback() {
    ...
    if (this.midiActiveTimeout) {
      window.clearTimeout(this.midiActiveTimeout);
    }
  }
  ...
  override render() {
    ...
    return html`<div class=${classes}>
      ...
      <div id="midi" @click=${this.toggleLearnMode}>
        ${this.learnMode ? 'Aprender' : `CC:${this.cc}`}
      </div>
    </div>`;
  }
}
